...
<template>
  <div class="m-5">
    <div v-if="position" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 min-w-0">
      <div @click="copy(latitude)" class="flex flex-col gap-3 text-center p-5 border justify-center">
        <span class="text-xl">Latitude</span>
        <span class="text-3xl" v-bind:title="locationAccuracy">{{ latitude }}</span>
      </div>
      <div @click="copy(longitude)" v-if="longitude" class="flex flex-col gap-3 text-center p-5 border justify-center">
      ...
      <div @click="copy(JSON.stringify(geoJSON, null, 2))" v-if="geoJSON" class="flex flex-col p-5 border justify-center"><span class="text-xl mb-3 text-center">GeoJSON</span> <pre class="overflow-x-scroll shadow-inner p-3">{{ geoJSON }}</pre></div>
      <div @click="copy(wktWithAltitude)" v-if="altitude" class="flex flex-col text-center p-5 border justify-center"><span class="text-xl mb-3">WKT</span> <pre class="overflow-x-scroll shadow-inner p-3">{{ wktWithAltitude }}</pre></div>
      <div @click="copy(wkt)" v-else class="flex flex-col text-center p-5 border justify-center"><span class="text-xl mb-3">WKT</span> <pre class="overflow-x-scroll shadow-inner p-3">{{ wkt }}</pre></div>
      ...
      <div @click="copy(timestamp)" v-if="timestamp" class="flex flex-col text-center p-5 border justify-center"><span class="text-xl mb-3">Last updated</span> <span class="text-3xl" v-bind:title="timestamp">{{ relativeTimestamp }} seconds ago</span></div>
    </div>
