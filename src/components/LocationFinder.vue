...
<template>
  <div class="m-5">
    <div v-if="position" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 min-w-0">
      ...
      <div @click="copy(altitude)" v-if="altitude" class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">Altitude</span> <span class="text-3xl mb-3">{{ altitude }} meters</span><span>+-{{ altitudeAccuracy }} meters</span></div>
      <div @click="copy(heading)" class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">Heading</span> <span class="text-3xl" v-html="heading"></span></div>
      <div @click="copy(mph)" v-if="mph" class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">Miles per hour</span> <span class="text-3xl">{{ mph }} mph</span></div>
      <div @click="copy(kph)" v-if="kph" class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">Kilometers per hour</span> <span class="text-3xl">{{ kph }} km/h</span></div>
      <div @click="copy(speed)" v-if="speed" class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">Meters per second</span> <span class="text-3xl">{{ speed }} m/s</span></div>
      <div @click="copy(JSON.stringify(geoJSON, null, 2))" v-if="geoJSON" class="flex flex-col p-5 rounded-xl border justify-center"><span class="text-xl mb-3 text-center">GeoJSON</span> <pre class="overflow-x-scroll shadow-inner p-3">{{ geoJSON }}</pre></div>
      <div @click="copy(wktWithAltitude)" v-if="altitude" class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">WKT</span> <pre class="overflow-x-scroll shadow-inner p-3">{{ wktWithAltitude }}</pre></div>
      <div @click="copy(wkt)" v-else class="flex flex-col text-center rounded-xl p-5 border justify-center"><span class="text-xl mb-3">WKT</span> <pre class="overflow-x-scroll shadow-inner p-3">{{ wkt }}</pre></div>
